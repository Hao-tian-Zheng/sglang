...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Select the backend for all-to-all communication for expert parallelism. `nccl` is an eager, variable-split fallback validated for `Qwen3MoeForCausalLM` on NVIDIA CUDA. It uses the Triton MoE runner, forces EP to span TP (`ep_size=tp_size`), and disables decode and prefill CUDA graphs. LoRA, EPLB, redundant experts, TBO/SBO overlap, speculative decoding, and enforced shared-expert fusion are not supported.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>`none`</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}><code>none</code>, <code>deepep</code>, <code>mooncake</code>, <code>nixl</code>, <code>mori</code>, <code>ascend_fuseep</code>, <code>flashinfer</code>, <code>megamoe</code>, <code>pplx</code>, <code>nccl</code></td>
    </tr>
        <tr>
